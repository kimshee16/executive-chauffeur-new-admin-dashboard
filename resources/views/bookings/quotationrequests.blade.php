    ...
    <table id="quotationrequeststable" class="table table-striped" style="width:100%">
        ...
        <tbody>
            @foreach ($quotationsData as $data)
                <tr>
                    <td><a class="btn btn-danger btn-sm" href="#" title="Delete" data-bs-toggle="modal" data-bs-target="#bookingDeleteConfirmModal" onclick="getBookingDetailsForDelete({{ $data->id }});"><i class="fa fa-trash"></i></a> <a class="btn btn-primary btn-sm" target='_blank' href="{{url('/clients?openDetails=1&clientid='.$data->client_id)}}" title="Client Details"><i class="fa fa-user"></i></a> <a class="btn btn-primary btn-sm" href="#" title="Confirm Proposal" data-bs-toggle="modal" data-bs-target="#bookingProposalConfirmModal" onclick="getBookingDetailsForProposalConfirm({{ $data->id }});"><i class="fa fa-envelope-open"></i></a> <a class="btn btn-primary btn-sm" href="#" title="Details" data-bs-toggle="modal" data-bs-target="#bookingEditModal" onclick="getBookingDetails({{ $data->id }});"><i class="fa fa-search"></i></a></td>
...
@include('modals/bookingproposalconfirmmodal')
@include('footer')
...
<script type="text/javascript">
    ...
    function getBookingDetailsForProposalConfirm(id) {
        ...
        $.ajax({
            ...
            url: baseurl + "/getbookingajax?id="+id,
            success: function(data) {
                document.getElementById("confirmto").value = data.data[0].email;
                document.getElementById("confirmsubject").value = "Our Proposal for Executive Chauffeur Hire on "+data.data[0].journey_date+" for "+data.data[0].first_name+" "+data.data[0].surname+".";

                CKEDITOR.replace("confirmbody");

                function setCKEditorContent(content) {
                  const editor = CKEDITOR.instances.confirmbody;
                  if (editor) {
                    editor.setData(content);
                  }
                }

                const contentToSet = `Dear `+data.data[0].first_name+` `+data.data[0].surname+`, <br><br>
                    Many thanks for your enquiry. <br><br> 
                    The price for providing Executive Chauffeur service on `+data.data[0].journey_date+` at `+data.data[0].journey_hour+`:`+data.data[0].journey_minute+` `+data.data[0].journey_AMPM+` from `+data.data[0].address+` to `+data.data[0].going_to+` will be &#163;`+data.data[0].book_total+` This is a fully inclusive price which includes the journey, meet and greet service, any waiting time and credit card processing costs. <br><br>
                    Upon confirmation we will schedule the booking with an executive vehicle (all our vehicles have Interior leather, wood trim, and Sat Navigation), together with mature chauffeur who will be suited. `+data.data[0].airport+` I shall provide contact mobile number upon confirmation. In terms of payment we are able to accept all major credit cards (except Diners) or cash. Meanwhile, If you have any queries please do not hesitate to contact me.<br><br>
                    To confirm booking and to secure the vehicle/chauffeur as quoted please reply to <a href="mailto:booking@executive-chauffeur.com">booking@executive-chauffeur.com</a><br>
                    To get an idea of what our executive cars look like please go here 
                    <a href="https://www.executive-chauffeur.com/BMW_7series.pdf">https://www.executive-chauffeur.com/BMW_7series.pdf</a> or 
                    <a href="https://www.executive-chauffeur.com/Voyager.pdf">https://www.executive-chauffeur.com/Voyager.pdf</a>
                    <br><br>
                    Best wishes and thank you for thinking of us, <br><br>
                    Amrit<br>
                    Customer Services  <br>
                    <table cellspacing="0" border="0" cellpadding="0" style="font-family:Tahoma; font-size:12px;">
                        <tr>
                            <td colspan="3"><img src="https://executive-chauffeur.com/wp-content/uploads/2018/10/ExecutiveChauffeurLogo.png"></td>
                            <td>&nbsp;</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td>Bookings:&nbsp;&nbsp;</td>
                            <td><a href="mailto:booking@executive-chauffeur.com">booking@executive-chauffeur.com</a>&nbsp;&nbsp;</td>
                            <td>Tel:&nbsp;  0330 22 33330</td>
                            <td>&nbsp;</td>
                        </tr>
                        <tr>
                            <td>Web:</td>
                            <td><a href="http://www.executive-chauffeur.com">www.executive-chauffeur.com</a></td>
                            <td>Int\'l Tel: +44 1753 740529</td>
                            <td>&nbsp;</td>
                        </tr>
                        <tr>
                            <td>Email:</td>
                            <td><a href="mailto:info@executive-chauffeur.com">info@executive-chauffeur.com</a></td>
                            <td>&nbsp;</td>
                            <td>&nbsp;</td>
                        </tr>        
                    </table> `;
                setCKEditorContent(contentToSet);
    ...
    if(window.location.href.includes("confirmedbookings")) {
        ...
        document.getElementById("confirmbookinglocation").value = "confirmedbookings";
        ...
        document.getElementById("confirmbookinglocation").value = "completedbookings";
    } else {
        ...
        document.getElementById("confirmbookinglocation").value = "quotationrequests";
    }
